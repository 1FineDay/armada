...
class ChartBuilderException(exception_client.ExceptionClient):
...
class IgnoredFilesLoadException(exception_client.ExceptionClient):
	'''Generic Exception that occurs during Pre-Flight checks'''
	
	message = 'An error occured while loading the ignored files in \".helmignore\"'

class MetadataLoadException(exception_client.ExceptionClient):
	''' Exception that occurs when metadata loading fails'''

	message = 'Failed to load metadata from chart yaml file'

class HelmChartBuildException(exception_client.ExceptionClient):
	'''Exception that occurs when Helm Chart fails to build'''

	def __init__(self, chart_name):
		self._chart_name

		self._message = 'Failed to build Helm chart for ' + self._chart_name + '.'

	super(HelmChartBuildException, self).__init__(self._message)

class DependencyException(exception_client.ExceptionClient):
	'''Exception that occurs when dependencies cannot be resolved for a Helm Chart'''

	def __init__(self, chart_name):
		self._chart_name

		self._message = 'Failed to resolve dependencies for ' + self._chart_name + '.'
